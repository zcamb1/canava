...
async def chat(query, userid, conid):
    ...
    for mess in conversation.messages[-10:]:
        if mess.sender == 'user':
            conversation_history.append('Human: '+ mess.text)
        elif mess.sender == 'bot':
            conversation_history.append('AI: '+ mess.text)
    conversation_history = '\n'.join(conversation_history)
    return StreamingResponse(answering(embed_model, driver, similarity_search, query, conid, conversation_history), media_type='text/plain') 
...
@app.post("/conversation/{conversation_id}/upload_file", summary="Upload txt file")
async def upload_file_conversation(conversation_id: str, file: UploadFile = File(...)):
    """
    Endpoint to upload a .txt file for a specific conversation.
    Files are stored in conversation_uploads/{conversation_id}/ folder.
    """
    try:
