...
from SuperAITech.agentic_rag.src.agent.similarity import Similarity
from SuperAITech.agentic_rag.src.agent.utils import irrelevant_context, graph_context, similarity_context
from neo4j import GraphDatabase
from llama_index.embeddings.huggingface import HuggingFaceEmbedding

def call_agent_stream(url, api_key, payload):
  url = url
  headers = {
      "x-api-key": api_key,
...
def call_agent_stream2(url, api_key, payload):
  url = url
  headers = {
      "x-api-key": api_key,
      "Content-Type": "application/json"
...
def call_agent(url, api_key, payload):
  ...
  response = requests.post(url, headers=headers, json=payload)
  return response.json()['outputs'][-1]['outputs'][-1]['results']['text']['text']
...
def router(driver, similarity_search, question, session_id, conversation):
  ...
  #   }
  # }
  # result = call_agent_stream2(url, api_key, payload)
...
def answering(embed_model, driver,similarity_search, ques, session_id, conversation):
  ...
  with open('context_minhhieuph.txt', 'w', encoding='utf-8') as fw:
      fw.write(context)
      fw.close()
  url = 'https://agent.sec.samsung.net/api/v1/run/6ed2a2dc-6b00-4caa-9345-dfa21dabda7f?stream=true'
  api_key = 'sk-NnCY6MlP7p0czCvg0uoPY-DYEdZa44YSjKoCXWI2ukM'
  payload = {
  "component_inputs": {
    "TextInput-3QsJw": {
  ...
  resp = call_agent_stream(url, api_key, payload)
  yield from resp
  return resp
